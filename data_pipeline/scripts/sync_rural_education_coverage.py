...
ANALYTICAL_FILENAMES = (*RAW_FILENAMES, "population_estimates.json", "rural_enrollments.json")
...
def _snapshot_digest(directory: Path) -> str:
    ...
    for filename in ANALYTICAL_FILENAMES:
        ...
        if not path.is_file():
            raise FileNotFoundError(f"Snapshot incompleto: {path}.")
...
def _stage_snapshot(
    ...
    *,
    ...
    snapshot_digest = _snapshot_digest(staging)
    manifest = {
        "schemaVersion": 1,
...
def replace_tables(
    ...
    *,
    ...
    with engine.begin() as connection:
        ...
        connection.execute(text(f"DELETE FROM {POPULATION_TABLE} WHERE ano_censo = 2022"))
        connection.execute(
            text(f"DELETE FROM {ENROLLMENT_TABLE} WHERE ano = ANY(:years)"),
            {"years": list(years)},
        )
        population.to_sql(
            POPULATION_TABLE,
        ...
        population_count = connection.execute(
            text(f"SELECT count(*) FROM {POPULATION_TABLE} WHERE ano_censo = 2022")
        ).scalar_one()
        enrollment_count = connection.execute(
            text(f"SELECT count(*) FROM {ENROLLMENT_TABLE} WHERE ano = ANY(:years)"),
            {"years": list(years)},
        ).scalar_one()
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=DATA_PIPELINE_DIR / "data" / "rural_education_coverage",
    )
    ...
    try:
        ...
        if args.apply:
            replace_tables(population_rows, enrollment_rows, years=years)
        result = {
            **manifest,
